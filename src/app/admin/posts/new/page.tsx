...
function AutoSaveLabel({ status, lastSaved }: { status: AutoSaveStatus; lastSaved: Date | null }) {
  ...
  if (status === "saved" && lastSaved) return (
    <span className="flex items-center gap-1.5 text-[9px] font-bold text-emerald-500 uppercase tracking-wider">
      <CheckCircle className="w-3 h-3" /> Draft saved at {lastSaved.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
    </span>
  );
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[9px] font-bold uppercase tracking-[0.25em] text-accent-600 mb-0.5">New Article</p>
            <h1 className="text-2xl font-bold text-brand-900 tracking-tight">Write Post</h1>
          </div>
        </div>
      ...
      {/* Autosave restored notice */}
      {draftId && (
        <div className="p-2.5 bg-emerald-50 border border-emerald-100 text-emerald-700 rounded-lg text-[10px] font-bold flex items-center gap-2">
          <Clock className="w-3.5 h-3.5" />
          Draft auto-saved — your work is safe.
        </div>
      )}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_300px] gap-5">
        ...
        <div className="space-y-4">
          ...
          <TipTapEditor
            content={content}
            onChange={setContent}
            placeholder="Start writing your story. Use the toolbar above to format text, add headings, links, and more…"
          />
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-brand-100/60 p-5">
            ...
            <div className="space-y-3">
              ...
              {/* Manual save draft button inside panel too */}
              <button
                onClick={() => doAutoSave()}
