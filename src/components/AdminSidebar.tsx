...
function NavLinks({ collapsed, filteredNavItems, pathname, onClickItem }: {
  collapsed: boolean;
  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed top-4 left-4 z-50 w-10 h-10 bg-white border border-slate-200 rounded-lg shadow-sm flex items-center justify-center hover:bg-slate-50 text-slate-600 transition-colors"
      ...
      <aside
        className={`lg:hidden fixed inset-y-0 left-0 z-[70] w-[260px] bg-white border-r border-slate-200/80 flex flex-col h-screen transform transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <nav className="flex-1 py-4 px-3 space-y-0.5 overflow-y-auto">
          ...
          {filteredNavItems.map((item) => {
            ...
            return (
              ...
              >
                <item.icon className={`w-4 h-4 flex-shrink-0 ${active ? "text-accent-600" : "text-slate-400"}`} />
                <span className="text-[11px] font-medium uppercase tracking-wider whitespace-nowrap">{item.label}</span>
                {active && <span className="ml-auto w-1 h-1 rounded-full bg-accent-600" />}
              </Link>
            );
          })}
        ...
        {/* Bottom */}
        <div className="border-t border-slate-100 py-4 px-3 space-y-0.5">
          <Link
            href="/"
            onClick={() => setMobileOpen(false)}
            className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-slate-500 hover:text-slate-900 hover:bg-slate-50 transition-all"
          >
            <Home className="w-4 h-4 flex-shrink-0 text-slate-400" />
            <span className="text-[11px] font-medium uppercase tracking-wider">View Site</span>
          </Link>
          <button
            onClick={() => signOut({ callbackUrl: "/" })}
            className="w-full flex items-center gap-3 rounded-lg px-3 py-2.5 text-slate-500 hover:text-red-600 hover:bg-red-50 transition-all"
          >
      ...
      {/* Desktop sidebar */}
      <aside
        className={`hidden lg:flex relative flex-shrink-0 bg-white border-r border-slate-200/80 flex-col h-screen sticky top-0 overflow-hidden transition-all duration-300 ease-in-out ${
